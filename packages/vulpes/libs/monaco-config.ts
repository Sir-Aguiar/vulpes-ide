...
export function registerPortugolLanguage(monaco: any) {
  const alreadyRegistered = monaco.languages
    .getLanguages()
    .some((lang: any) => lang.id === "portugol");
  ...
  if (!alreadyRegistered) {
    ...
    monaco.languages.setMonarchTokensProvider("portugol", {
      // Tokens não reconhecidos ficam sem estilo. Erros reais vão para
      // setModelMarkers, que exibem mensagem ao passar o mouse.
      defaultToken: "",
...
export interface IEditorCompileError {
  message: string;
  line: number;
  column: number;
}

export function applyCompileErrorMarkers(
  monacoInstance: {
    editor: {
      setModelMarkers: (
        model: unknown,
  ...
  editor: { getModel: () => unknown } | null,
  errors: IEditorCompileError[],
) {
  ...
  monacoInstance.editor.setModelMarkers(
    ...
    errors.map((error) => {
      ...
      const column = Math.max(1, (error.column || 0) + 1);
      ...
      return {
        ...
        endLineNumber: line,
        endColumn: column + 1,
        message: error.message || "Erro de compilação",
